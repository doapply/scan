...
import (
	...
	awscommands "github.com/doapply/search/pkg/cloud/aws/commands"
	"github.com/doapply/search/pkg/commands/artifact"
	"github.com/doapply/search/pkg/commands/server"
	"github.com/doapply/search/pkg/fanal/analyzer"
	"github.com/doapply/search/pkg/flag"
	k8scommands "github.com/doapply/search/pkg/k8s/commands"
	"github.com/doapply/search/pkg/log"
	"github.com/doapply/search/pkg/module"
	"github.com/doapply/search/pkg/plugin"
	"github.com/doapply/search/pkg/policy"
	"github.com/doapply/search/pkg/types"
)

// VersionInfo holds the search DB version Info
...
func NewRootCommand(version string, globalFlags *flag.GlobalFlagGroup) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "search [global flags] command [flags] target",
  ...
  $ search image python:3.4-alpine

  # Scan a container image from a tar archive
  $ search image --input ruby-3.1.tar
...
func NewImageCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
  ...
  $ search image --severity HIGH,CRITICAL alpine:3.15

  # Ignore unfixed/unpatched vulnerabilities
  $ search image --ignore-unfixed alpine:3.15

  # Scan a container image in client mode
  $ search image --server http://127.0.0.1:4954 alpine:latest

  # Generate json result
  $ search image --format json --output result.json alpine:3.15

  # Generate a report in the CycloneDX format
  $ search image --format cyclonedx --output result.cdx alpine:3.15`,
...
func NewFilesystemCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
  ...
  $ search fs /path/to/your_project

  # Scan a single file
  $ search fs ./search-ci-test/Pipfile.lock`,
		PreRunE: func(cmd *cobra.Command, args []string) error {
...
func NewRootfsCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
  ...
  $ search rootfs /tmp/rootfs

  # Scan from inside a container
  $ docker run --rm -it alpine:3.11
  / # curl -sfL https://raw.githubusercontent.com/doapply/search/main/contrib/install.sh | sh -s -- -b /usr/local/bin
  / # search rootfs /`,
		PreRunE: func(cmd *cobra.Command, args []string) error {
...
func NewRepositoryCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
  ...
  $ search repo https://github.com/knqyf263/search-ci-test`,
		PreRunE: func(cmd *cobra.Command, args []string) error {
...
func NewClientCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			log.Logger.Warn("'client' subcommand is deprecated now. See https://github.com/doapply/search/discussions/2119")
...
func NewServerCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
  ...
  $ search server --listen 0.0.0.0:10000
`,
...
func NewKubernetesCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
  ...
  $ search k8s --report summary cluster

  # namespace scanning:
  $ search k8s -n kube-system --report summary all

  # resources scanning:
  $ search k8s --report=summary deploy
  $ search k8s --namespace=kube-system --report=summary deploy,configmaps

  # resource scanning:
  $ search k8s deployment/orion
`,
		PreRunE: func(cmd *cobra.Command, args []string) error {
...
`, strings.Join(services, "\n- ")),
  ...
  $ search aws --region us-east-1

  # limit scan to a single service:
  $ search aws --region us-east-1 --service s3

  # limit scan to multiple services:
  $ search aws --region us-east-1 --service s3 --service ec2

  # force refresh of cache for fresh results
  $ search aws --region us-east-1 --update-cache
`,
		PreRunE: func(cmd *cobra.Command, args []string) error {
...
func NewVMCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
  ...
  $ search vm --scanners vuln ami:${your_ami_id}

  # Scan your AWS EBS snapshot
  $ search vm ebs:${your_ebs_snapshot_id}
`,
		PreRunE: func(cmd *cobra.Command, args []string) error {
...
func NewSBOMCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
  ...
  $ search sbom /path/to/report.cdx

  # Scan CycloneDX and generate a CycloneDX report
  $ search sbom --format cyclonedx /path/to/report.cdx

  # Scan CycloneDX-type attestation and show the result in tables
  $ search sbom /path/to/report.cdx.intoto.jsonl
`,
		PreRunE: func(cmd *cobra.Command, args []string) error {
